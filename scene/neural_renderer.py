...
class GridRenderer(nn.Module):
    def __init__(self,
        ...
        # print(f"self.in_dim_dir: {self.in_dim_dir}")
        # print(f"self.in_dim_dir + self.geo_feat_dim: {self.in_dim_dir + self.geo_feat_dim}")
        self.color_net = MLP(self.in_dim_dir + self.geo_feat_dim, 3, self.hidden_dim_color, self.num_layers_color)

    
    def create_encoder(self):
        encoder_x, in_dim_x = get_encoder(
    ...
    def Encode_x(self, x):
        ...
        x_normalized = (x - self.coord_center + self.bound) / (2 * self.bound)
        return self.encoder_x(x_normalized)
    ...
    def color(self, sigma_result, d):
        ...
        # print(f"enc_d.shape: {enc_d.shape}, geo_feat.shape: {geo_feat.shape}")
